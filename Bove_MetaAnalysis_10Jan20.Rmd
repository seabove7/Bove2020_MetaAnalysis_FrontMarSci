---
title: "Meta-analysis reveals reduced coral calcification under projected ocean warming but not acidification across the Caribbean"
output:
...
*The following script and analyses were written by Colleen Bove and James Umbanhowar. This version of the script was last committed to GitHub on 11 January 2020*
...
ggsave("Figure2_GlobalCI_10Jan20.pdf", width = 5, height = 3.5)
...
w.global$mag.cen <- w.global$mag - mean(w.global$mag) # center days


w.global.mod <- rma.mv(y,v, mods = ~ mag.cen + irradiance.cen + Seawater + Feeding + day.cen, random = list(~ (1|study), ~ (1|spec)), data = w.global) 
...
a.global$mag.cen <- a.global$mag - mean(a.global$mag) # center days

a.global.mod <- rma.mv(y,v, mods = ~ mag.cen + irradiance.cen + Seawater + Feeding + day.cen, random = list(~ (1|study), ~ (1|spec)), data = a.global) 
...
rownames(a.global.out) <- c(" Intercept", " Magnitude", " Irradiance", " Seawater", " Feeding (3x)", " Feeding (N.D.)", " Duration")
...
full.param <- rbind(w.global.out, a.global.out)

param.tab <- kable(full.param, digits = 4) %>% 
...
Corals from Belize only exhibited clearly reduced calcification rates under ocean warming (**Figure 3A and Supplementary Table 3**), while acidification, warming, and the combination of stressors did not clearly alter experimental calcification rates of corals from the Florida Keys (**Figure 3B and Supplementary Table S3**). Further, the resulting *Q~E~* suggests there is significant between-study variation (**Supplementary Tables S3**).
...
ggsave("Figure3_RegionalCI_10Jan20.pdf", width = 8, height = 3.5)
...
####(c.)	Temperature and aragnoite saturation state impacts on calcification rates across studies
Secondary analysis of mean calcification rates (mg cm^−2^ day^−1^) against treatment temperature across all studies revealed a parabolic response to temperature (**Figure 4 and Supplementary Tables S4, S5**), with modelled calcification rates higher for corals collected from the Florida Keys than those from Belize. Similarly, mean calcification rates (mg cm^−2^ day^−1^) across Ω~Arag~ resulted in a nonlinear response to acidification (**Figure 5 and Supplementary Tables S6, S7**), again with modelled calcification rates higher in the Florida Keys than in Belize. Both nonlinear trends in response to temperature and Ω~Arag~ were a result of treatment rather than region, suggesting regional differences identified in the meta-analysis were due to experimental designs employed to represent current regional environmental differences. 
...
ggsave("Figure4_CalcWarm_30Dec19.pdf", width = 7, height = 4)
...
ggsave("FigureS7_CalcAcid_10Jan20.pdf", width = 7, height = 4)
...
ggsave("FigureS2_OAfunnel_10Jan20.pdf", width = 7, height = 6)
...
ggsave("FigureS1_OAforest_10Jan20.pdf", width = 11, height = 12)
...
ggsave("FigureS3_OWforest_26Sep19.pdf", width = 9, height = 10)
...
ggsave("FigureS4_OWfunnel_26Sep19.pdf", width = 7, height = 5)
...
ggsave("FigureS5_bothforest_10Jan20.pdf", width = 9, height = 8)
...
ggsave("FigureS6_bothfunnel_10Jan20.pdf", width = 7, height = 6)
