...
```
*The following script and analyses were written by Colleen Bove and James Umbanhowar. This version of the script was last committed to GitHub on 23 December 2019*
...
g.mod5 <- rma.mv(y,v, mods = ~ treat * mag + irradiance + Feeding, random = list(~ (1|study), ~ (1|species)), data = df2) # using this model
g.mod6 <- rma.mv(y,v, mods = ~ treat * mag + days + Feeding, random = list(~ (1|study), ~ (1|species)), data = df2) # using this model
g.mod7 <- rma.mv(y,v, mods = ~ treat * mag + days + irradiance, random = list(~ (1|study), ~ (1|species)), data = df2) # using this model
...
global.mod <- rma.mv(y,v, mods = ~ treat * mag + days + irradiance + Feeding, random = list(~ (1|study), ~ (1|species)), data = df2)
...
global_CI <- 
  ...
  #ylim(-1.6,0.6)+
  geom_point(aes(x = treat, y = pred), size=10, shape=95)
...
region.mod <- rma.mv(y,v, mods = ~ treat * region * mag + days + irradiance + Feeding, random = list(~ (1|study), ~ (1|species)), data = df2a)
...
table3 <- kable(region.mod.tab, digits = 4, booktabs = T, caption = "Table 3. Regional Model Output") %>%
  kable_styling(font_size = 14, full_width = F) %>% 
  pack_rows("Model Results", 1, 16) %>%
  #pack_rows("Florida", 17, 20) %>%
  pack_rows("Variance Components", 17, 20)
table3
...
Here, I am including magnitude only. I think I will add feeding, irradiance, and duration after I solidify how to proceed with constructing the predition dataframe with factors! 
...
James -- I am not confident that this was the correct way to construct the predicted values dataframe just based on the resulting plotted model. Let me know your thoughts!
...
[ Model: rate ~ temperature + region + temperature^2^ + magnitude + (1  | study) + (1 | spec), weights = 1 / SE ]
...
#### Ocean acidification calcification rate against aragonite saturation state analysis (included in supplemental materials)

**I stopped here because I wanted to make sure that I did the warming right.**
...
acid.rate2.mod <- lmer(rate ~ scale.arag + I(scale.arag^2) + p.mag + (1|study) + (1|spec), weights = 1/SE, data = calc.arag,REML=F) 
...
acid.pred <- data.frame(acid=seq(1.7,5.8,length.out=100),pred=predict(acid.rate2.mod, newdata = data.frame(scale.arag=seq(min(calc.arag$scale.arag),max(calc.arag$scale.arag),length.out=100), p.mag=seq(min(calc.arag$p.mag),max(calc.arag$p.mag),length.out=100)),re.form=NA))
...
tab_model(acid.rate2.mod, show.r2 = FALSE, show.aicc = TRUE, title = "Table 4. Mixed-effects model output of effects of aragonite saturation state on calcification rates")
```
