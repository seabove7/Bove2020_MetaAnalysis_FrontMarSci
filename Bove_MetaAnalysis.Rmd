...
I've added multiple models of warming effects, including region effects and quadratic effects.  The best model seems to be the model with quadratic temperature effects, but no region effects, suggesting that some of the region effects may be the result of different temperatures.
...
```{r echo=FALSE}
global_OW_calc <-
ggplot(calc.warm, aes(x=temp, y=rate, colour=region))+ geom_point(aes(size=1/SE),shape=1)+
...
Once again, comparing several models of the effect.  Here we still see a quadratic effect, but strangely, it seems to see minimum rate in the intermediate pco2.  The quadratic effect is small and when you look at the confidence intervals, both intervals overlap 0, suggesting that they are highly correlated, really suggesting that it is very difficult to differentiate among positive and negative values.  Some of this is backed up by the fact that the pure intercept model is a relatively good fit.
...
acid.rate2.mod <- lmer(rate ~ scale.pco2  + I(scale.pco2^2) + (1|study) + (1|spec), weights = 1/SE, data = calc.acid,REML=F) 

acid.rate.mod <- lmer(rate ~ scale.pco2 + (1|spec), weights = 1/SE, data = calc.acid,REML=F) 
summary(acid.rate.mod) # view summary output of model

acid.rate2.region.mod <- lmer(rate ~ scale.pco2  + region+ I(scale.pco2^2) + (1|study) + (1|spec), weights = 1/SE, data = calc.acid,REML=F) 

acid.rate.region.mod <- lmer(rate ~ scale.pco2 + region+(1|spec), weights = 1/SE, data = calc.acid,REML=F) 
summary(acid.rate.mod) # view summary output of model

acid.region.mod <- lmer(rate ~ region + (1|study) + (1|spec), weights = 1/SE, data = calc.acid,REML=F) 

acid.intercept.mod <-lmer(rate ~ (1|study) + (1|spec), weights = 1/SE, data = calc.acid,REML=F) 

AICc(acid.intercept.mod,acid.rate.mod,acid.rate2.mod,acid.rate.region.mod,acid.rate2.region.mod,acid.region.mod)
...
acid.rate2.mod <- lmer(rate ~ scale.pco2  + I(scale.pco2^2) + (1|study) + (1|spec), weights = 1/SE, data = calc.acid)
...
acid.pred <- data.frame(pco2=seq(min(calc.acid$pco2),max(calc.acid$pco2),length.out=100),pred=predict(acid.rate2.mod, newdata = data.frame(scale.pco2=seq(min(calc.acid$scale.pco2),max(calc.acid$scale.pco2),length.out=100)),re.form=NA))

global_OA_calc <-
ggplot(acid.df, aes(x=pco2, y=rate, colour=region))+ geom_point(aes(size=1/SE),shape=1)+
    theme(panel.grid.major=element_blank(), legend.position="bottom", panel.grid.minor=element_blank(), panel.background=element_blank())+
  theme(axis.line=element_line(color="black"))+
  theme(legend.key=element_blank())+
